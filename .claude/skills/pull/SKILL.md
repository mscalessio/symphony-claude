---
description: Pull latest origin/main into the current local branch and resolve merge conflicts; use when asked to sync, update branch, or merge origin/main.
...
8. Verify with project checks (run tests, typecheck).
...
- Inspect context before editing:
  ...
  - Summarize the intent of both changes, decide the semantically correct
    outcome, then edit:
    - State what each side is trying to achieve.
    - Identify the shared goal, if any, and whether one side supersedes the other.
    - Decide the final behavior first; only then craft the code to match.
...
- For generated files, resolve non-generated conflicts first, then regenerate:
  - Prefer resolving source files and handwritten logic before touching
    generated artifacts.
  - Run the tooling command that produced the generated file to recreate it
